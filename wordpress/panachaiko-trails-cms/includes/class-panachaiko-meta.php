...
final class Panachaiko_Trails_Meta {
    ...
    public static function add_meta_boxes(): void {
        add_meta_box( 'panachaiko_trail_details', 'Στοιχεία διαδρομής', array( __CLASS__, 'render_trail_meta_box' ), 'trail', 'normal', 'high' );
        add_meta_box( 'panachaiko_poi_details', 'Στοιχεία σημείου διαδρομής', array( __CLASS__, 'render_poi_meta_box' ), 'trail_poi', 'normal', 'high' );
    }

    private static function register_trail_meta(): void {
        $common = array( 'single' => true, 'show_in_rest' => true, 'auth_callback' => static fn() => current_user_can( 'edit_posts' ) );
        register_post_meta( 'trail', 'trail_code', $common + array( 'type' => 'string', 'sanitize_callback' => 'sanitize_text_field' ) );
        register_post_meta( 'trail', 'trail_stage', $common + array( 'type' => 'string', 'sanitize_callback' => array( __CLASS__, 'sanitize_trail_stage' ) ) );
        register_post_meta( 'trail', 'trail_color', $common + array( 'type' => 'string', 'sanitize_callback' => 'sanitize_hex_color' ) );
    ...
    private static function register_poi_meta(): void {
        $common = array( 'single' => true, 'show_in_rest' => true, 'auth_callback' => static fn() => current_user_can( 'edit_posts' ) );
        register_post_meta( 'trail_poi', 'related_trail', $common + array( 'type' => 'integer', 'sanitize_callback' => 'absint' ) );
        register_post_meta( 'trail_poi', 'content_type', $common + array( 'type' => 'string', 'sanitize_callback' => array( __CLASS__, 'sanitize_content_type' ) ) );
        register_post_meta( 'trail_poi', 'poi_type', $common + array( 'type' => 'string', 'sanitize_callback' => array( __CLASS__, 'sanitize_poi_type' ) ) );
        foreach ( array( 'latitude', 'longitude' ) as $field ) {
            register_post_meta( 'trail_poi', $field, $common + array( 'type' => 'number', 'sanitize_callback' => array( __CLASS__, 'sanitize_float' ) ) );
        }
    ...
    public static function render_trail_meta_box( WP_Post $post ): void {
        ...
        $stage = (string) get_post_meta( $post->ID, 'trail_stage', true ) ?: 'planned';
        $color = (string) get_post_meta( $post->ID, 'trail_color', true ) ?: '#84a06e';
        ...
        <div class="panachaiko-meta-grid">
            <div><label for="panachaiko_trail_code">Κωδικός διαδρομής</label><input id="panachaiko_trail_code" name="panachaiko_trail_code" type="text" value="<?php echo esc_attr( $code ); ?>" /></div>
            <div><label for="panachaiko_trail_stage">Κατάσταση</label><select id="panachaiko_trail_stage" name="panachaiko_trail_stage"><option value="existing" <?php selected( $stage, 'existing' ); ?>>Υπάρχει</option><option value="planned" <?php selected( $stage, 'planned' ); ?>>Σχεδιάζεται</option><option value="investigation" <?php selected( $stage, 'investigation' ); ?>>Υπό διερεύνηση</option></select></div>
            <div><label for="panachaiko_trail_color">Χρώμα</label><input id="panachaiko_trail_color" name="panachaiko_trail_color" type="color" value="<?php echo esc_attr( $color ); ?>" /></div>
            <div><label for="panachaiko_length_km">Μήκος (χλμ)</label><input id="panachaiko_length_km" name="panachaiko_length_km" type="number" step="0.01" min="0" value="<?php echo esc_attr( $length ); ?>" /></div>
            <div><label for="panachaiko_elev_min">Ελάχιστο υψόμετρο (μ)</label><input id="panachaiko_elev_min" name="panachaiko_elev_min" type="number" value="<?php echo esc_attr( $elev_min ); ?>" /></div>
            <div><label for="panachaiko_elev_max">Μέγιστο υψόμετρο (μ)</label><input id="panachaiko_elev_max" name="panachaiko_elev_max" type="number" value="<?php echo esc_attr( $elev_max ); ?>" /></div>
            <div><label for="panachaiko_gain_m">Ανάβαση (μ)</label><input id="panachaiko_gain_m" name="panachaiko_gain_m" type="number" min="0" value="<?php echo esc_attr( $gain ); ?>" /></div>
    ...
    public static function render_poi_meta_box( WP_Post $post ): void {
        ...
        $content_type = (string) get_post_meta( $post->ID, 'content_type', true ) ?: 'note';
        $type = (string) get_post_meta( $post->ID, 'poi_type', true ) ?: 'general';
        ...
        ?>
        <div class="panachaiko-meta-grid">
            <div class="wide"><label for="panachaiko_related_trail">Σχετική διαδρομή</label><select id="panachaiko_related_trail" name="panachaiko_related_trail"><option value="0">— Χωρίς συσχέτιση —</option><?php foreach ( $trails as $trail ) : $trail_code = (string) get_post_meta( $trail->ID, 'trail_code', true ); ?><option value="<?php echo esc_attr( $trail->ID ); ?>" <?php selected( $related, $trail->ID ); ?>><?php echo esc_html( trim( $trail_code . ' — ' . $trail->post_title, ' —' ) ); ?></option><?php endforeach; ?></select></div>
            <div><label for="panachaiko_content_type">Περιεχόμενο</label><select id="panachaiko_content_type" name="panachaiko_content_type"><option value="note" <?php selected( $content_type, 'note' ); ?>>Περιγραφή</option><option value="photo" <?php selected( $content_type, 'photo' ); ?>>Φωτογραφία</option><option value="video" <?php selected( $content_type, 'video' ); ?>>Βίντεο</option></select></div>
            <div><label for="panachaiko_poi_type">Κατηγορία σημείου</label><select id="panachaiko_poi_type" name="panachaiko_poi_type"><?php foreach ( $types as $value => $label ) : ?><option value="<?php echo esc_attr( $value ); ?>" <?php selected( $type, $value ); ?>><?php echo esc_html( $label ); ?></option><?php endforeach; ?></select></div>
            <div><label for="panachaiko_latitude">Γεωγραφικό πλάτος (lat)</label><input id="panachaiko_latitude" name="panachaiko_latitude" type="number" step="0.000001" value="<?php echo esc_attr( $lat ); ?>" /></div>
    ...
    public static function save_trail_meta( int $post_id ): void {
        if ( ! self::can_save( $post_id, 'panachaiko_trail_meta_nonce', 'panachaiko_save_trail_meta' ) ) return;
    ...
    public static function save_poi_meta( int $post_id ): void {
        ...
        self::save_number( $post_id, 'latitude', 'panachaiko_latitude' ); self::save_number( $post_id, 'longitude', 'panachaiko_longitude' );
        if ( isset( $_POST['panachaiko_external_video_url'] ) ) { $url = esc_url_raw( wp_unslash( $_POST['panachaiko_external_video_url'] ) ); $url ? update_post_meta( $post_id, 'external_video_url', $url ) : delete_post_meta( $post_id, 'external_video_url' ); }
    ...
    private static function save_text( int $post_id, string $meta_key, string $field_name ): void { if ( ! isset( $_POST[ $field_name ] ) ) return; $value = sanitize_text_field( wp_unslash( $_POST[ $field_name ] ) ); '' === $value ? delete_post_meta( $post_id, $meta_key ) : update_post_meta( $post_id, $meta_key, $value ); }
    private static function save_number( int $post_id, string $meta_key, string $field_name ): void { if ( ! isset( $_POST[ $field_name ] ) ) return; $raw = wp_unslash( $_POST[ $field_name ] ); if ( '' === $raw || ! is_numeric( $raw ) ) { delete_post_meta( $post_id, $meta_key ); return; } update_post_meta( $post_id, $meta_key, (float) $raw ); }
    private static function save_stage( int $post_id ): void { $value = isset( $_POST['panachaiko_trail_stage'] ) ? sanitize_text_field( wp_unslash( $_POST['panachaiko_trail_stage'] ) ) : 'planned'; update_post_meta( $post_id, 'trail_stage', self::sanitize_trail_stage( $value ) ); }
    private static function save_color( int $post_id ): void { $value = isset( $_POST['panachaiko_trail_color'] ) ? sanitize_hex_color( wp_unslash( $_POST['panachaiko_trail_color'] ) ) : ''; update_post_meta( $post_id, 'trail_color', $value ?: '#84a06e' ); }
    public static function sanitize_trail_stage( $value ): string { return in_array( $value, array( 'existing','planned','investigation' ), true ) ? $value : 'planned'; }
    public static function sanitize_content_type( $value ): string { return in_array( $value, array( 'note','photo','video' ), true ) ? $value : 'note'; }
